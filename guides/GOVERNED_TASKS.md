...
> **Landing in 2.0 / on the v2 preview.** Everything on this page ships on the v2 preview (release candidate `2.0.0rc1`, `mcp==2.0.0b2`) and is **not** in released `1.6.2`. Tasks were promoted out of `mcp.server.experimental` into a first-class negotiated extension only in the SDK v2 beta, which is what makes governed relay buildable at all (see [ADR-014](../adr/ADR-014-tasks-relay-with-governance.md)). On the stable `1.6.x` line the observed behavior is still a clean `TaskRelayNotSupported` rejection of any upstream task handle.
