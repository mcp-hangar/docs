...
- **`mcp.server.id` changes meaning within one trace.** On the live tool-call path: logical target on `batch.call.<tool>`, `concurrency.acquire`, `policy.check_access`, `approval_gate.check`, `approval_gate.flow` and `invoke_with_retry`; selected member on `mcp_server.cold_start` and `command.send.InvokeToolCommand`. Of the spans on the executor's call path, only that member-valued pair has to change. Two lifecycle spans that landed after this ADR was first written (#1547, #1571) also name the member inside a group call's trace: `mcp_server.startup_wait` (`infrastructure/observability/startup_spans.py:170-171`) and `mcp_server.launch` (`infrastructure/launchers/base.py:38-40`). `mcp_server.startup_wait` is opened by the startup observer for a caller waiting on a start, through single flight or the aggregate's readiness wait (`startup_spans.py:99-103`, `:130-140`), and `mcp_server.launch` by the launcher. Both act on one concrete server process keyed by its own id and shared across calls (`server/tools/batch/executor.py:2360-2367`). Line references in the route and lifecycle passages of this ADR are to `src/mcp_hangar/` on mcp-hangar `main` at `c607e838`.
...
| `hangar.route.backend`, `hangar.route.reason` | the selected member, or on a standalone call the server itself; `standalone`, `load_balanced`, `pinned`, `canary`, `canary_fallback`, `no_available_member` |

Two route cases are settled here rather than by a new value. A tenant pinned to a member that is out of rotation reads `hangar.route.reason=canary_fallback`: pins are configured under the group's `canary:` block (`server/config.py:1331-1353`), resolved by the same `CanaryPolicy` as the canary split (`domain/model/mcp_server_group.py:41-50`), and fall back to the load balancer on the same path (`domain/model/mcp_server_group.py:590-604`), so there is no separate pin-fallback value. A standalone call carries `hangar.route.backend` equal to `mcp.server.id`, so the key is present whenever a backend was chosen and a query by backend needs no fallback to `mcp.server.id`.
...
- **`mcp.server.id` means the logical target on every span on the executor's call path:** `batch.call.<tool>`, `policy.check_access`, `approval_gate.check`, `approval_gate.flow`, `concurrency.acquire`, `invoke_with_retry`, `mcp_server.cold_start` and `command.send.InvokeToolCommand`. One attribute that means two things within a call cannot be queried; the concrete member is `hangar.route.backend`. The logical target is what the caller named and the scope the call is governed under (the selected member's own policy is added to it, `server/tools/batch/executor.py:1956-1961`), and audit is to be keyed on it too (#1594), so it is the identity that has to be constant across the call. The member is a routing outcome, a separate fact: calls to one group spread across its members by load balancing, canary split and failover, and a group call refused with `no_available_member` has no member at all. The executor selects the member once per call, in `_gate_resolve_target` (`server/tools/batch/executor.py:1939-1945`), and fixes the dispatch target before `retry_sync` runs (`executor.py:2517`, `:2559`), so every retry attempt of one call reaches the same member. A failure reported against that member can take it out of rotation for the *next* call's selection (`executor.py:2202-2203`, `domain/model/mcp_server_group.py:733`, `:583`), not for a retry of the current one. **This is a compatibility change, not an additive one:** `mcp_server.cold_start` and `command.send.InvokeToolCommand` carry the member today, and queries reading them as the concrete backend must move to `hangar.route.backend`. `batch.call.<tool>` carries both `hangar.route.backend` and `hangar.route.reason`; `mcp_server.cold_start` and each `command.send.InvokeToolCommand` attempt carry `hangar.route.backend` only; a `no_available_member` refusal carries the reason and no backend. #1286 owns it and ships it with a changelog and migration note, and that note also states that the lifecycle spans below keep naming the member.
- **Lifecycle spans name the server they act on.** `mcp_server.launch` and `mcp_server.startup_wait` carry the concrete server in `mcp.server.id`, which for a group call is the selected member. The launcher and the startup observer act on one concrete server process keyed by its own id and shared across calls (`server/tools/batch/executor.py:2360-2367`, `infrastructure/launchers/base.py:38-40`, `infrastructure/observability/startup_spans.py:170-171`), and a member can belong to several groups (`fastmcp_server/flat_tool_projection.py:1150-1153`), so one start can serve calls that named different logical targets and has no single one to name. The member of a group call stays discoverable through `hangar.route.backend` on `batch.call.<tool>`, `mcp_server.cold_start` and `command.send.InvokeToolCommand`.
...
| **Group routing** — logical target differs from the selected member | — | `mcp.server.id` is the logical target on every span on the executor's call path, `mcp_server.cold_start` and `command.send.InvokeToolCommand` included; `hangar.route.backend` carries the member on `batch.call.<tool>`, `mcp_server.cold_start` and each `command.send.InvokeToolCommand` attempt; `mcp_server.launch` and `mcp_server.startup_wait` name the member they start |
...
The status of refused spans changes across the Hangar-owned tree, not only on `batch.call.<tool>`; dashboards and alerts keyed on ERROR must be reviewed, including the Helm-owned ones in mcp-hangar/helm-charts. **`mcp.server.id` changes value on two spans** — a compatibility change, not an additive one, and queries reading it there as the concrete backend must move to `hangar.route.backend`. It is constant across the spans on the executor's call path, not across the whole trace: `mcp_server.launch` and `mcp_server.startup_wait` still name the member inside a group call's trace, so a query by `mcp.server.id` over every span of a trace sees two values unless it is scoped to the call's spans or uses `hangar.route.backend`. A trace whose entry span is ERROR above an UNSET `batch.call.<tool>` is now an expected shape on the flat path, and anyone reading the entry span alone will still count governance denials as errors. Telling a refusal exception from a failure inside the tracer is new work that #1285 and #1295 both depend on. `hangar.*` is new surface to keep stable, and revision stays absent for most decision types.
...
- mcp-hangar/mcp-hangar#1275 (this decision's issue), #1276 (the data-handling contract), #1264 (the tracing epic), #1286 (route telemetry, where the scope of the logical-target rule and the one selection per call were settled).
