...
`pip install mcp-hangar` resolves to **2.2.0**, the current stable release. It
...
```bash
docker pull ghcr.io/mcp-hangar/mcp-hangar:2.2.0

# Run with config
docker run -v $(pwd)/config.yaml:/app/config.yaml:ro \
  ghcr.io/mcp-hangar/mcp-hangar:2.2.0
```
