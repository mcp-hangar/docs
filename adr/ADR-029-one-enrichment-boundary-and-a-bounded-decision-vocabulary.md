...
- **A refusal looks like a failure.** A refusing gate returns a `CallResult` with `success=False`, and `executor.py:1556-1561` marks the span ERROR for any such result — the same status an upstream failure gets. This contradicts the #1272 rule that only operational failures are ERROR.
- **`mcp.server.id` changes meaning within one trace.** Logical target on `batch.call.<tool>`, `concurrency.acquire`, `policy.check_access`, `approval_gate.check` and `invoke_with_retry`; selected member on `mcp_server.cold_start` and `command.send.InvokeToolCommand`.
...
- **Relationship to [ADR-007](ADR-007-langfuse-integration.md):** its port and adapter design stands. Its Decision 2, the decorator as the place where tool spans start, is superseded in part: the Langfuse adapter is driven from this boundary instead.

### 5. A bounded outcome and reason vocabulary under `hangar.`

| Key | Values | Task |
| --- | --- | --- |
| `hangar.gate.name` | `access`, `withdrawal`, `digest_pin`, `circuit_breaker`, `validator`, `approval`, `egress_l7`, `rate_limit`, `cold_start` | #1285 |
...
| `hangar.l7.mode`, `hangar.l7.rule_kind` | `audit`, `enforce`; `tool`, `argument`, `header` | #1295 |
...
- **A refusal is a correct answer, not an operational failure.** It sets `hangar.gate.outcome=deny` and leaves span status UNSET. ERROR is reserved for operational failures (#1272, #1277). This changes the status of `batch.call.<tool>` for refused calls and ships in **one release** with #1285, with a changelog note that error-rate panels stop counting denials and `hangar.gate.outcome=deny` is the new query.
...
- **The log-side refusal notion widens to match**, or is documented as deliberately L7-only: `_log_call_failure` treats only the two L7 errors as refusals today.
...
One boundary, one vocabulary and one parent/link rule unblock #1278, #1285, #1286, #1295, #1297 and #1279, #1281, #1288, #1296 — none has to invent its own. Denials stop inflating error rates, and the first refusing gate becomes queryable. The dead path ADR-007 pointed at is retired rather than revived.
...
Hangar depends on the SDK middleware staying on by default. The #1310 harness asserts this as a contract test.
