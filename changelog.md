...
**Tenant-aware governance is now enforced on the live call path.** MCP Hangar
1.4.0 extends the 1.3 front-door model with per-tenant digest pins, multi-issuer
OAuth trust, resource-bound JWT audiences, and tenant-sticky group routing.

- **Per-tenant digest pinning** -- configure tenant-specific tool schema pins under
  `tool_projection.tenant_overrides.<tenant>.pins`; mismatches are evaluated on
  the invocation path and recorded with `tenant_id`.
- **Resource-bound OAuth** -- when `auth.oidc.resource_uri` is configured, accepted
  JWTs must use that URI as `aud` (RFC 8707), matching the RFC 9728 Protected
  Resource Metadata `resource` value.
- **Multi-issuer OIDC trust** -- `auth.oidc.issuers` can list multiple trusted
  authorization servers with issuer-specific audiences, JWKS URIs, claim mappings,
  and token lifetime limits.
- **Per-tenant canary and version routing** -- groups can pin tenants to specific
  members or send a deterministic percentage of tenants to a canary member before
  falling back to the configured load-balancing strategy.
- **Governance observability** -- new Grafana dashboard and alert groups cover
  cost, security, capability, enforcement, concurrency, availability, discovery,
  and remote transport signals.

### Added

- **core:** per-tenant canary and version routing for groups (#283), closes #275 #226
- **core:** per-tenant tool digest pinning on the call path (#276), closes #233 #226
- **observability:** activate availability and transport alerts (#269), closes #268
- **observability:** add governance dashboard and alerts for cost, security, and concurrency metrics (#267), closes #261 #266
- **security:** bind token audience to resource URI (RFC 8707) (#274), closes #255 #253
- **security:** multi-issuer trust registry for OAuth Resource Server (#273), closes #254 #253

### Fixed

- **core:** cost counters emit a doubled `_total` suffix (#266), closes #265
- **core:** harden per-tenant digest pinning (per-server enforcement, tenant_id, tests) (#280), closes #278 #226
- **core:** select a group member on the invoke path (#282), closes #281 #275
- **observability:** align monitoring dashboards and alerts with `mcp_server` rename (#263), closes #260
- **security:** reject non-string `iss` claim instead of raising 500 (#279), closes #277

### Changed

- **observability:** remove dead `ObservabilityMetrics` registry (#272), closes #271
