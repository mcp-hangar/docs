...
| `circuit_breaker.failure_threshold` | `int` | `10` | >= 1 | Failures in a row, across the group's members, before the circuit opens. A success ends the run. The circuit has no reset timer: it closes once `min_healthy` members are back in rotation |
