...
- **`mcp.server.id` changes meaning within one trace.** On the live tool-call path: logical target on `batch.call.<tool>`, `concurrency.acquire`, `policy.check_access`, `approval_gate.check`, `approval_gate.flow` and `invoke_with_retry`; selected member on `mcp_server.cold_start` and `command.send.InvokeToolCommand`. Of the spans the executor opens for a call, only that member-valued pair has to change. Two lifecycle spans that landed after this ADR was first written (#1547, #1571) also name the member inside a group call's trace: `mcp_server.startup_wait` (`infrastructure/observability/startup_spans.py:170-171`) and `mcp_server.launch` (`infrastructure/launchers/base.py:38-40`). They are opened by the single-flight start and the launcher, not by the executor, and both act on one concrete server keyed by its own id (`server/tools/batch/executor.py:2360-2367`). Line references in the route and lifecycle passages of this ADR are to `src/mcp_hangar/` on mcp-hangar `main` at `c607e838`.
...
- **`mcp.server.id` means the logical target on every span the executor opens for a call:** `batch.call.<tool>`, `policy.check_access`, `approval_gate.check`, `approval_gate.flow`, `concurrency.acquire`, `invoke_with_retry`, `mcp_server.cold_start` and `command.send.InvokeToolCommand`. One attribute that means two things within a call cannot be queried; the concrete member is `hangar.route.backend`. The logical target is what the caller named and what policy, quota and audit are keyed on, so it is the identity that has to be constant across the call. The member is a routing outcome, a separate fact: calls to one group spread across its members by load balancing, canary split and failover, and a group call refused with `no_available_member` has no member at all. The executor selects the member once per call, in `_gate_resolve_target` (`server/tools/batch/executor.py:1939-1945`), and fixes the dispatch target before `retry_sync` runs (`executor.py:2517`, `:2559`), so every retry attempt of one call reaches the same member. A failure reported against that member can take it out of rotation for the *next* call's selection (`executor.py:2202-2203`, `domain/model/mcp_server_group.py:732`, `:583`), not for a retry of the current one. **This is a compatibility change, not an additive one:** `mcp_server.cold_start` and `command.send.InvokeToolCommand` carry the member today, and queries reading them as the concrete backend must move to `hangar.route.backend`. #1286 owns it and ships it with a changelog and migration note, and that note also states that the lifecycle spans below keep naming the member.
- **Lifecycle spans name the server they act on.** `mcp_server.launch` and `mcp_server.startup_wait` carry the concrete server in `mcp.server.id`, which for a group call is the selected member. The launcher and the single-flight start act on one concrete server process keyed by its own id (`server/tools/batch/executor.py:2360-2367`, `infrastructure/launchers/base.py:38-40`, `infrastructure/observability/startup_spans.py:170-171`), and a member can belong to several groups (`fastmcp_server/flat_tool_projection.py:1150-1153`), so one start can serve calls that named different logical targets and has no single one to name. The member of a group call stays discoverable through `hangar.route.backend` on the call's spans.
...
| **Group routing** — logical target differs from the selected member | — | `mcp.server.id` is the logical target on every span the executor opens for the call, `mcp_server.cold_start` and `command.send.InvokeToolCommand` included; `hangar.route.backend` carries the member on the spans that selected it; `mcp_server.launch` and `mcp_server.startup_wait` name the member they start |
...
The status of refused spans changes across the Hangar-owned tree, not only on `batch.call.<tool>`; dashboards and alerts keyed on ERROR must be reviewed, including the Helm-owned ones in mcp-hangar/helm-charts. **`mcp.server.id` changes value on two spans** — a compatibility change, not an additive one, and queries reading it there as the concrete backend must move to `hangar.route.backend`. It is constant across the spans the executor opens for a call, not across the whole trace: `mcp_server.launch` and `mcp_server.startup_wait` still name the member inside a group call's trace, so a query by `mcp.server.id` over every span of a trace sees two values unless it is scoped to the call's spans or uses `hangar.route.backend`. A trace whose entry span is ERROR above an UNSET `batch.call.<tool>` is now an expected shape on the flat path, and anyone reading the entry span alone will still count governance denials as errors. Telling a refusal exception from a failure inside the tracer is new work that #1285 and #1295 both depend on. `hangar.*` is new surface to keep stable, and revision stays absent for most decision types.
